...
public class DriveTrain extends SubsystemBase {
    ...
    public void CMDTeleOp(CommandXboxController driveController, CommandXboxController armController) {
        ...
        if (driveController.leftBumper().getAsBoolean()) {
            isFast = true;
            doDrive(-1 * driveController.getLeftY(), driveController.getRightX(), driveController.getLeftX(), RobotMath.getSqRtValue(driveController.getRightTriggerAxis()) * Constants.DriveConstants.superHyperMaxSpeed);
        } else {

            isFast = false;
            doDrive(-1 * driveController.getLeftY(), driveController.getRightX(), driveController.getLeftX(), RobotMath.getSqRtValue(driveController.getRightTriggerAxis()) * Constants.DriveConstants.maxSpeed);
            //}
            if (!bDone) {
 ...
 */
    ...
    public void cmdDriveForward(double distance, double speed) {
        ...
        driveForwardStartAngle = RobotContainer.getInstance().gyro.getNormalizedNavxAngle();
        bDone = false;
        resetMotorEncoders();
    ...
    public void driveForwardStraight() {
        if (getDistanceForward() >= driveForwardDistance) {
            ...
            double gyroTurnCorrection = RobotMath.calcTurnRate(RobotContainer.getInstance().gyro.getNormalizedNavxAngle(), driveForwardStartAngle, Constants.DriveConstants.driveForwardProportion);
