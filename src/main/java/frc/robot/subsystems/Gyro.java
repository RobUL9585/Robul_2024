...
public class Gyro extends SubsystemBase {
    ...
    public double getNormalizedNavxAngle() {
    ...
    public double gyroNormalize(double heading) {
        ...
        if (degrees < -179) degrees = degrees + 360;
    ...
    public boolean gyroInTol(double currHeading, double desiredHeading, double tol) {
        ...
        if (signumUpperTol > 0 && signumLowerTol > 0) {
            if ((normalCurr >= lowerTol) && (normalCurr <= upperTol)) retValue = true;
        }
    ...
    public double deltaHeading(double currHeading, double targetHeading) {
        ...
        // Positive value
        if (currHeading >= 0 && targetHeading >= 0) returnValue = targetHeading - currHeading;
            // one of each
        else if (currHeading >= 0 && targetHeading <= 0) returnValue = (targetHeading + currHeading);
            // one of each again
        else if (currHeading <= 0 && targetHeading >= 0) returnValue = -1 * (targetHeading + currHeading);
            // both negative
        else if (currHeading <= 0 && targetHeading <= 0) returnValue = targetHeading - currHeading;
    ...
    public double deltaHeading(double targetHeading) {
        return deltaHeading(getNormalizedNavxAngle(), targetHeading);
    }
